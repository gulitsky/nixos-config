  ...
  config = lib.mkIf cfg.enable {
    ...
    environment.persistence."/persist" = {
      ...
      directories = [
        "/etc/nixos"
        "/etc/ssh"
        # Вся база v2rayA: аккаунт веб-морды, узлы, подписки, выбранные
        # правила маршрутизации. Каталог по умолчанию у него в /etc, а не
        # в /var/lib, — и без персиста каждая загрузка встречает экраном
        # «зарегистрируйте администратора» с пустым списком серверов.
        "/etc/v2raya"
        "/var/lib/bluetooth"
        "/var/lib/fprint"
