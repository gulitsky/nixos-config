...
}:
{
  ...
  # Zed — графический редактор рядом с терминальным Fresh. Модулем, а не
  # пакетом в home.packages: модуль умеет extraPackages (см. ниже) и умеет
  # доливать настройки в settings.json, не забирая файл себе.
  programs.zed-editor = {
    enable = true;

    # Тулчейн Go для Zed: поддержка языка у него встроенная (расширение не
    # нужно), но gopls он ищет в PATH, а не найдя — скачивает свой бинарник
    # в ~/.local/share/zed. extraPackages оборачивает zeditor и дописывает
    # эти пакеты в конец PATH: редактор видит их при запуске из лончера, а
    # в каталоге с direnv версии из devShell стоят раньше и побеждают.
    # Глобально в home.packages их не кладу — в терминале Go приходит из
    # devShell проекта.
