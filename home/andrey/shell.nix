{ pkgs, ... }:
{
  ...
  programs.yazi = {
    ...
    # Монтирование внешних накопителей. Сам плагин ничего не монтирует: список
    # берёт из `lsblk`, монтирует через `udisksctl`, извлекает `eject` — все
    # три уже в системном PATH, потому что services.udisks2 включён.
    # Флешку смонтирует без пароля (политика udisks2 разрешает съёмные
    # носители активной сессии), на внутренний раздел упрётся в polkit и
    # предложит sudo — у плагина для этого есть свой ввод пароля.
    ...
    keymap.mgr.prepend_keymap = [
      {
        on = "M";
        run = "plugin mount";
        desc = "Монтирование внешних накопителей (m/u/e внутри)";
      }
    ];
  };
}
