...
- на шаге выбора диска — **Load driver**, второй привод, `amd64\w11\viostor.inf`,
  иначе списка дисков не будет; сеть (`NetKVM`) ставится оттуда же;
- после установки — `virtio-win-guest-tools.exe` с того же привода: буфер обмена,
  подгон разрешения, balloon;
